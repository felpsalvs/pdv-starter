...
1. Copie esta pasta (`soparia-pdv`) para o computador da soparia.
...
1. Abra a pasta `soparia-pdv`, botão direito → "Abrir no Terminal".
2. Digite:
   ```
   npm start
   ```
...
2. Dentro da pasta `soparia-pdv`, copie o arquivo `printer.config.example.json`
